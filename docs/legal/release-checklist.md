...
- `npm test`：server 43、film-core 8、微信 19、Web/发布工具 25，全部通过。
- `npm run check`：107 个 JavaScript 文件语法通过；在已导出 ESP-IDF 5.5.2 + Python 3.14.2 的严格环境中，契约门禁为 `235 passed / 0 pending / 0 failed`。基于 `00bdb65` 构建隔离修复提交的源码副本已完成 STD/Pro/Max 三机型构建，应用分区余量为 13%/12%/14%。构建不等于实体刷写验收，完整证据见 [`docs/ops/verification-matrix.md`](../ops/verification-matrix.md)。
