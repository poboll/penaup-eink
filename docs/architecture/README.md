...
这是面向少量设备和单人维护的模块化单体，而不是提前拆分的微服务。SQLite、媒体目录和事件表可以完整备份；当规模需要时，媒体可迁移到对象存储，任务队列可迁移到独立 worker，但 HTTP 契约和 `packages/film-core` 不变。`/health` 只表示进程存活，`/readyz` 会额外检查 SQLite 与媒体目录的读写能力。
