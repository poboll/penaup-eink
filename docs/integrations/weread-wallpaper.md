...
“用示例数据预览”完全在浏览器内生成三本虚构书籍和一段虚构阅读轨迹，不会发送请求，也不会把示例当作微信读书数据。它与真实流程共用同一套系统常用字默认排版、可选汇文明朝体、本地 Worker、Pro 尺寸和状态提示，适合第一次打开页面时先体验。
...
网页四种场景默认使用普通系统字体，显影前可以切换到仓库内的 `apps/web/fonts/huiwen-mincho.woff2`；小程序默认使用系统常用字，并在页面中保留同样的可选字体语义，不把未经核实授权的 OTF 放进小程序包。Web 字体是将用户提供的汇文明朝体字体转换成网页可加载格式后的资源；原始 OTF 不进入 Git，字体的再分发权利仍需在公开发布前单独核对。
...
屏保排版默认使用系统常用字，用户可在开始显影前选择 `apps/web/fonts/huiwen-mincho.woff2`（汇文明朝体 Web 子集）；正文和技术标签使用系统无衬线/等宽字体。该字体由本机 `/Users/Apple/Downloads/Huiwenmingchaoti/汇文明朝体.otf` 生成，仓库只提交 Web 所需子集，不提交原始 OTF；字体授权和再分发边界仍要在发布前逐项复核，见 [权利与来源说明](../legal/provenance.md)。
...
只有用户选择汇文明朝体时，Canvas 才会在绘制前等待 `Huiwen Mincho` WebFont 加载；系统常用字路径不额外加载字体。BLE 写入完成后页面使用“等待电子纸刷新确认”状态；写入进度不等同于设备已经刷新成功。
