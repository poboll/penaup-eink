...
| JavaScript / 契约 | PASS with external pending | `npm run check`：93 个 JS 文件语法通过；契约 `204 passed / 1 pending / 0 failed` |
...
| ESP-IDF 三机型构建 | PENDING | 本机有 ESP-IDF 5.5.2 `export.sh`，但当前 shell `idf.py` 不存在，不能声称 STD/Pro/Max 构建通过 |
...
如果 `idf.py` 仍未出现在 `PATH`，命令失败是环境门禁失败，不是固件构建证据。
