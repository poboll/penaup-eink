...
| 用户-facing 机型 | 文件头宽 × 高 | 像素主体 | 完整文件 | 像素布局 |
|---|---:|---:|---:|---|
| 花生片 Penaup / STD | 600 × 400 | 120,000 B | 120,032 B | 列优先并翻转 |
| 花生片 Penaup Pro | 792 × 528 | 209,088 B | 209,120 B | 行优先 |
| 花生片 Penaup Max | 1,200 × 1,600 | 960,000 B | 960,032 B | 行优先，双面板分半发送 |
...
用户创作画布与文件主体的坐标转换由机型配置决定：

- STD：本地画布按竖向体验编辑，写入主体时使用 `(x × height) + (height - 1 - y)`；
- Pro：使用 `(y × width) + x`；
- Max：使用行优先，固件把每行前半发送给 CS0、后半发送给 CS1。
