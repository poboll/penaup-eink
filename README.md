...
当前主产品规格是花生片 Pro：E6 3.68 英寸相纸按手持方向呈现为 `528 × 792` 竖向视觉画布；设备内部继续接收历史兼容的 `792 × 528` `.film` 协议。STD 与 Max 仍保留为可显式选择的机型，未连接设备时 Web、小程序和模板默认以 Pro 纸面开始创作。
...
npm run service:install # macOS：交给 launchd 常驻 8787，不随临时终端退出
npm run service:status  # 查看 launchd 是否仍在运行
```
...
如果只是临时开发可以使用 `npm start`；如果希望页面在 Codex、编辑器或终端关闭后仍可打开，macOS 使用 [本地 8787 常驻服务说明](docs/ops/local-dev-service.md) 安装用户级 LaunchAgent。它默认只监听 `127.0.0.1`，不会暴露到局域网。

工作台的“读书”入口是一间小型壁纸实验室：可以把微信读书的本周小票、本月日历、书架标本或单本读书卡排成花生片 Pro 的 3.68 英寸屏保。页面采用接近白色的草纸背景，普通系统字体是默认 UI，生成时可选择系统常用字或汇文明朝体；服务端只做一次性数据转发，浏览器本地完成纸面排版和六色显影。叠色、网点和抖动会让六种物理墨水形成最多 48 种色彩观感，但不会改变 `.film` 的六色协议。Key 不写入仓库或服务端存储。视觉 token 与纸面噪点实现见 [类纸表面规范](docs/brand/paper-surface.md)，使用前请阅读 [微信读书屏保整合说明](docs/integrations/weread-wallpaper.md) 和 [壁纸实验室设计](docs/plans/2026-08-23-weread-wallpaper-lab-design.md)。
