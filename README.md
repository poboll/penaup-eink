...
工作台的“读书”入口是一间小型壁纸实验室：可以把微信读书的本周小票、本月日历、书架标本或单本读书卡排成花生片 Pro 的 3.68 英寸屏保。页面采用偏白糙米色的类纸背景，使用汇文明朝体做本地排版；服务端只做一次性数据转发，浏览器本地完成纸面排版和六色显影。叠色、网点和抖动会让六种物理墨水形成最多 48 种色彩观感，但不会改变 `.film` 的六色协议。Key 不写入仓库或服务端存储。视觉 token 与纸纤维实现见 [类纸表面规范](docs/brand/paper-surface.md)，使用前请阅读 [微信读书屏保整合说明](docs/integrations/weread-wallpaper.md) 和 [壁纸实验室设计](docs/plans/2026-08-23-weread-wallpaper-lab-design.md)。
